...
async def show_subscription_options(message: Message):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="5 тренировок -4% 4800₽", callback_data="sub:5"),
            InlineKeyboardButton(text="10 тренировок -10% 9000₽", callback_data="sub:10")
        ]
    ])
...
async def process_subscription(callback: CallbackQuery):
    ...
    payment_url = create_payment(
        ...
        payment_method="sbp",
        description=f"WhoopClub абонемент | {count} | sub:{subscription_id} | {payer}"
    )

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Оплатить", url=payment_url)],
        [InlineKeyboardButton(text="❌ Отменить", callback_data=f"user_cancel_sub:{subscription_id}")]
    ])
...
#sync def notify_admins(callback: CallbackQuery):
...
async def user_cancel_subscription(callback: CallbackQuery):
    ...
    with get_connection() as conn:
        ...
        if row[0] != "pending":
            await callback.answer("⚠️ Абонемент уже оплачен", show_alert=True)
            return
...
#sync def confirm_subscription(callback: CallbackQuery):
#   subscription_id = int(callback.data.split(":")[1])
...
#sync def reject_subscription(callback: CallbackQuery):
#   subscription_id = int(callback.data.split(":")[1])
#   admin_name = callback.from_user.full_name
#
#   with get_connection() as conn:
